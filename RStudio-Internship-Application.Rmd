...
Some examples of using ggplot2 extensions like `plotly` [@R-plotly] and `rayshader` [@R-rayshader] to make cool visualizations.
...
Plotting the % change in price ($) over the previous 24 hour period for the market as a whole:

```{r show_rayshader}
knitr::include_url("file:///C:/Users/Ricky/Desktop/RStudio-Internship-Application/rayshader.mp4")
```
...
```{r price_usd_btc_7d, message=FALSE, warning=FALSE}
...
```{r plotPickVariable, message=FALSE, warning=FALSE}
...
```{r 6h_change_7d_top5, message=FALSE, warning=FALSE}
...
```{r prev_to_target_24h, message=FALSE, warning=FALSE}
...
```{r 12h_change_7d_top100, echo=FALSE}
...
```{r 24h_change_by_date, echo=FALSE}
...
```{r rows_collected_by_algo, echo=FALSE}
