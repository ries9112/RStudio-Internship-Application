...
(IF I CAN'T ADJUST THE COLUMN SIZE FOR DESCRIPTION, PUT SCREENSHOTS IN HERE INSTEAD SINCE I ALREADY HAVE THEM ANYWAYS)
...
```{r}
exercise_data <- select(exercise_data, -date, -date_time_utc, -date_time, -pkDummy, -pkey, -price_usd_x_daysLater, -date_time_x_daysLater)
```
...
```{r}
lm_predictions <- crypto_test %>% add_predictions(lm_fitted)
```

```{r}
xgboost_predictions <- crypto_test %>% add_predictions(xgboost_fitted)
```
...
```{r}
#running into issues using method above, non-standardized method below works for accuracy metrics
lm_model <- lm(target_percent_change ~ ., data=crypto_train)
...
randomforest_model <- randomForest(target_percent_change ~ ., data=crypto_train)

glance(randomforest_model)
```

This article was before modelr came along, but good guide on using group_by and do() in dplyr to make one model for each cryptocurrency if I'm stuck: https://r4stats.com/2017/04/18/group-by-modeling-in-r-made-easy/

Also remember to consult this as a resource: https://r4ds.had.co.nz/many-models.html
